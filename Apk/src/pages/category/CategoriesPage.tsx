...
import { getAllCategories, getAllProducts, type Category, type Product } from '@/api';
...
function CategoriesPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setError('');
      try {
        const [categoryRes, productRes] = await Promise.all([getAllCategories(), getAllProducts(200)]);
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#f2f7ff] via-[#f7fafc] to-white text-foreground">
      <div className="px-4 pb-10 pt-6">
        <header className="mb-4 flex items-center justify-between rounded-2xl border border-slate-200/80 bg-white/90 px-3 py-2.5">
          <Link to="/" className="flex items-center gap-1 text-xs font-medium text-slate-600 hover:text-slate-900">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-[15px] font-semibold tracking-tight">Categories</p>
        ...
        <section className="mb-4 rounded-3xl bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#334155] px-4 py-5 text-white">
          <h1 className="text-[24px] font-semibold leading-[1.15] tracking-tight">Find products by category</h1>
          <p className="mt-2 text-[13px] leading-5 text-slate-200">Browse all sections and open the one you need.</p>
            ...
            {categories.map((category) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="line-clamp-1 text-[14px] font-semibold tracking-tight">{category.name}</p>
                  <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-slate-500">
                    {category.description || 'Explore this category'}
                  </p>
                  <p className="mt-2 text-[11px] font-medium text-[#334155]">
                    {productCountByCategory[category.category_id] ?? 0} products
                  </p>
                </div>
